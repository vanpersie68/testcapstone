<template>
  <div class="wrapper">
    <div class="post">
      <a v-if="isImage"  :href="question.articleURL" target="_blank"><img :src="question.articleImage" :alt="question.articleTitle" width="100%" height="auto" /></a>
    ...
    <div class="comment">
      <div class="comment-items">
        ...
        <div class="comment-content">
          ...
          <div style="margin-left: 10px;width: 100%;">
            <el-input placeholder="Please enter a user name" size="small" v-model="item.username" @blur="updateComment(item)"></el-input>
            <el-input placeholder="Please enter a comment" size="small" v-model="item.content" @blur="updateComment(item)"></el-input>
      ...
      <div class="type-in">
        <el-input v-model="comment" placeholder="add a comment"></el-input>
        <button-base
...
mounted(){
  ...
  methods: {
    ...
    async updateComment (it) {
      const res = await SurveyServices.patchComment(it.id, {content: it.content, username: it.username})
    },
    async addComment() {
      ...
      const res = await SurveyServices.postComment(this.question.id, {content: this.comment})
      console.log(res)
    ...
    async delComment(id) {
      ...
      this.question.comments.splice(this.question.comments.findIndex((el) => el.id===id), 1)
    },
    /**
